...
def parse_weapons(content: str) -> List[Dict]:
    ...
    for line in content.split("\n"):
        ...
        if in_weapons_section and "|" in line:
            ...
            if match:
                name, price_str, size_str, weight_str, damage = [
                ...
                is_ammunition = False
                if any(
                    x in name.lower() for x in ["arrow", "quarrel", "bullet", "stone"]
                ) and not any(x in name.lower() for x in ["bow", "crossbow", "sling"]):
                ...
                if (
                    not name
                    or name == "Thrown (one handed)"
                    or name == "Melee (one handed)"
                    or name == "Melee (two handed)"
                ):
                    continue
                ...
                weapons.append(
                    {
                        ...
                        "is_equippable": True,
                        "equip_slot": (
                            "main_hand" if item_type == ItemType.WEAPON else None
                        ),
...
def parse_armor(content: str) -> List[Dict]:
    ...
    for line in content.split("\n"):
        ...
        if in_armor_section and "|" in line:
            ...
            if match:
                name, price_str, weight_str, ac_str = [
                    g.strip() for g in match.groups()
                ]
