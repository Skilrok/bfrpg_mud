        ...
        if response.status_code == 200:
            ...
            if characters and len(characters) > 0:
                ...
                for idx, character in enumerate(characters):
                    print(f"{idx+1}. {character.get('name')} (ID: {character.get('id')})")
                return characters
            else:
...
def activate_character(token, character_id):
    ...
    try:
        ...
        if response.status_code in [200, 201, 204]:
            ...
            print(f"Failed to activate character: {response.status_code}: {response.text}")
            return False
    except Exception as e:
...
def explore_world(token, character_id):
    ...
    for cmd in commands:
        ...
        if not result:
            ...
            print(f"Command '{cmd}' didn't succeed. Message: {result.get('message', 'Unknown error')}")
