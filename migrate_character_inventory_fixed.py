                ...
                if isinstance(old_equipment, dict):
                    for slot, item_id in old_equipment.items():
                        if slot not in new_equipment or new_equipment[slot] != item_id:
                            ...
                            item_name_result = db.execute(text("""
                                SELECT name FROM items WHERE id = :item_id
                            """), {"item_id": item_id}).first()
                            
                            item_name = item_name_result[0] if item_name_result else "Unknown"
                            
                            logger.warning(f"  Expected: {slot} -> {item_name} (ID: {item_id})")
                            if slot in new_equipment:
                                new_item_name_result = db.execute(text("""
                                    SELECT name FROM items WHERE id = :item_id
                                """), {"item_id": new_equipment[slot]}).first()
                                
                                new_item_name = new_item_name_result[0] if new_item_name_result else "Unknown"
...
def fix_character_equipment(db, char_id, char_name, old_equipment):
    ...
    try:
        ...
        for slot, item_id in old_equipment.items():
            ...
            """), {"char_id": char_id, "item_id": item_id}).first()
