...
def check_database_users(db_file):
    ...
    try:
        ...
        # Get user data
        cursor.execute("SELECT id, username, email, is_admin, is_active, hashed_password FROM users")
        users = cursor.fetchall()
        
        print("\n=== USERS IN DATABASE ===")
        print(f"{'ID':^5} | {'Username':^20} | {'Email':^30} | {'Admin':^5} | {'Active':^6} | {'Password Hash Preview':^20}")
        print("-" * 100)
        
        for user in users:
            user_id, username, email, is_admin, is_active, hashed_pwd = user
            pwd_preview = hashed_pwd[:10] + "..." if hashed_pwd else "None"
            print(f"{user_id:^5} | {username:^20} | {email:^30} | {is_admin:^5} | {is_active:^6} | {pwd_preview:^20}")
