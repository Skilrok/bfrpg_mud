    ...
    with get_db_context() as db:
        try:
                    ...
                    if item_data.get("equipped", False):  # Only check equipped items
                        ...
                        for slot, equip_item_id in equipment.items():
                            if equip_item_id == item_id:
                                ...
                                print(f"Item {item_id} ({item_name}) properly in equipment slot: {slot}")
                                break
                        ...
                        if not is_in_equipment:
                            ...
                            if not slot:
                                if item_type == "weapon":
                                    ...
                                    print(f"Cannot determine slot for item {item_id} ({item_name}) of type {item_type}")
                                    continue
                            
                            print(f"Fixing: Adding {item_name} ({item_type}) to equipment in '{slot}' slot")
            ...
            for slot, item_id in equipment.items():
                ...
                if item_id_str in inventory and not inventory[item_id_str].get("equipped", False):
                    print(f"Item {item_id} in slot {slot} not marked as equipped in inventory")
