...
def test_unequip_armor(character_name):
    ...
    with get_db_context() as db:
        try:
            ...
            logger.info(
                f"After unequipping - AC: {updated_character.armor_class}, Expected: {expected_ac}"
            )
            if updated_character.armor_class != expected_ac:
                logger.error(
                    f"AC mismatch after unequipping: Got {updated_character.armor_class}, Expected {expected_ac}"
                )
            else:
