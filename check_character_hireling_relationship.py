...
def check_character_hireling_relationship():
    ...
    with get_db_context() as db:
        ...
        for character in characters:
            ...
            char_hirelings = db.query(Hireling).filter(Hireling.character_id == character.id).all()
            
            logger.info(f"Character {character.name} (ID: {character.id}) has {len(char_hirelings)} hirelings")
        ...
        for hireling in hirelings:
            if hireling.character_id:
                character = db.query(Character).filter(Character.id == hireling.character_id).first()
                character_name = character.name if character else "Unknown"
                logger.info(f"Hireling {hireling.name} (ID: {hireling.id}) belongs to character {character_name} (ID: {hireling.character_id})")
            else:
                logger.info(f"Hireling {hireling.name} (ID: {hireling.id}) is not assigned to any character")
...
def try_direct_sql_query():
    ...
    with get_db_context() as db:
        ...
        for row in result:
            ...
            logger.info(f"Character: {row[0]} ({row[1]}) -> Hireling: {row[2]} ({hireling_name})")
...
if __name__ == "__main__":
    ...
    if schema_ok:
        logger.info("Database schema is correct (character_id exists, master_id doesn't)")
    else:
