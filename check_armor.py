            ...
            if not equipment:
                ...
                for slot, item_id in equipment.items():
                    ...
                    if item:
                        ...
                        properties = json.dumps(item.properties) if item.properties else "{}"
                        print(f"Slot {slot}: {item.name} (ID: {item.id}, Type: {item_type}, Properties: {properties})")
                    else:
            ...
            for item_id_str, item_data in inventory.items():
                try:
                    ...
                    if item:
                        ...
                        if item_type == "armor":
                            armor_found = True
                            print(f"ARMOR: {item.name} (ID: {item.id}, Status: {status}, Properties: {properties})")
                        else:
                            print(f"Item: {item.name} (ID: {item.id}, Type: {item_type}, Status: {status})")
                    else:
