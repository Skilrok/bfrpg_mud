...
if os.path.exists(TEST_DB_PATH):
    try:
        ...
        TEST_DB_PATH = (
            f"test_authentication_{threading.get_ident()}_{uuid.uuid4().hex[:8]}.db"
        )
...
def test_login_success(client, test_db, test_user):
    ...
    existing_user = (
        test_db.query(models.User).filter(models.User.username == username).first()
    )
    if existing_user:
...
def test_login_invalid_username(client):
    ...
    response = client.post(
        "/api/auth/token",
        data={"username": "nonexistent_user", "password": "TestPassword123!"},
    )
...
def test_login_invalid_password(client, test_db):
    ...
    response = client.post(
        "/api/auth/token", data={"username": username, "password": "WrongPassword123!"}
    )
    assert response.status_code == 401
    assert "Incorrect username or password" in response.json()["detail"]
...
def test_protected_route_with_invalid_token(client):
    ...
    response = client.post(
        "/api/auth/logout", headers={"Authorization": "Bearer invalidtoken123"}
    )
    assert response.status_code == 401
