...
class TestLoginPage:
    ...
    def test_login_functionality(self, browser, api_client, auth_token):
        ...
        try:
            WebDriverWait(browser, 5).until(EC.url_to_be(f"{BASE_URL}/game.html"))
...
class TestGameInterface:
    ...
    def test_character_data_loaded(self, browser, api_client, auth_token, character_id):
        ...
        response = api_client.get(
            f"{API_URL}/characters/{character_id}", headers=headers
        )
        ...
        try:
            WebDriverWait(browser, 5).until(
                EC.text_to_be_present_in_element(
                    (By.ID, "character-name"), character_data["name"]
                )
            )
            ...
            assert (
                character_data["name"]
                in browser.find_element(By.ID, "character-name").text
            )
            assert (
                character_data["race"]
                in browser.find_element(By.ID, "character-race").text
            )
            assert (
                character_data["character_class"]
                in browser.find_element(By.ID, "character-class").text
            )
        except TimeoutException:
    ...
    def test_command_input(self, browser, api_client, auth_token, character_id):
        ...
        try:
            WebDriverWait(browser, 3).until(
                EC.text_to_be_present_in_element((By.CLASS_NAME, "game-output"), "look")
            )
...
class TestInventorySystem:
    def test_inventory_display(self, browser, api_client, auth_token, character_id):
        ...
        try:
            ...
            matching_items = [
                name
                for name in api_item_names
                if any(name in item_text for item_text in item_names)
            ]
    ...
    def test_equip_item(self, browser, api_client, auth_token, character_id):
        ...
        assert any(
            item_id == equippable_item["id"] for item_id in equipment.values()
