...
class TestBasicCommands:
    ...
    def test_unknown_command(self, game_session):
        ...
        assert (
            "unknown command" in output.lower() or "don't understand" in output.lower()
        )
...
class TestInventoryCommands:
    def test_equip_command(self, game_session, api_client, auth_token, character_id):
        ...
        if equippable_item:
            ...
            assert (
                "equipped" in output.lower()
                or "wearing" in output.lower()
                or "wielding" in output.lower()
            )
        else:
    ...
    def test_examine_command(self, game_session, api_client, auth_token, character_id):
        ...
        # Get inventory
        response = api_client.get(
            f"{API_URL}/items/inventory/{character_id}", headers=headers
        )
        inventory = response.json()
...
class TestMovementCommands:
    def test_direction_commands(self, game_session):
        ...
        for direction in directions:
            ...
            assert len(output) > 0
            assert (
                direction in output.lower()
                or "can't go" in output.lower()
