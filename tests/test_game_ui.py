...
# REMOVED: from selenium.common.exceptions import TimeoutException
...
class TestLoginPage:
    ...
    def test_login_page_styling(self, browser):
        ...
        bg_color = browser.execute_script(
            "return window.getComputedStyle(arguments[0]).backgroundColor", terminal
        )
        text_color = browser.execute_script(
            "return window.getComputedStyle(arguments[0]).color", terminal
        )
        ...
        assert (
            "0, 255, 0" in text_color
            or "255, 255, 255" in text_color  # Green
            or "255, 191, 0" in text_color  # White  # Amber
        )
    ...
    def test_login_visual_effects(self, browser):
        ...
        effects = browser.find_elements(
            By.CSS_SELECTOR, ".scanline, .flicker, .glow, .terminal-effect"
        )
...
class TestGameInterface:
    ...
    def test_game_terminal_styling(self, game_session):
        ...
        font_family = browser.execute_script(
            "return window.getComputedStyle(arguments[0]).fontFamily", terminal
        )
        ...
        assert any(
            font in font_family.lower()
            for font in ["monospace", "courier", "consolas", "lucida"]
        )
    ...
    def test_command_input_styling(self, game_session):
        ...
        text_color = browser.execute_script(
            "return window.getComputedStyle(arguments[0]).color", command_input
        )
        border = browser.execute_script(
            "return window.getComputedStyle(arguments[0]).border", command_input
        )
...
class TestGameOutput:
    ...
    def test_output_formatting(self, game_session):
        ...
        line_height = browser.execute_script(
            "return window.getComputedStyle(arguments[0]).lineHeight", output_element
        )
        font_size = browser.execute_script(
            "return window.getComputedStyle(arguments[0]).fontSize", output_element
        )

        # Line height should be reasonable for terminal output
        # Convert values to numbers for comparison
        line_height_value = (
            float(line_height.replace("px", ""))
            if "px" in line_height
            else float(line_height)
        )
...
class TestResponsiveness:
    def test_mobile_viewport(self, browser, api_client, auth_token, character_id):
        ...
        browser.execute_script(
            f"localStorage.setItem('characterId', '{character_id}');"
        )
