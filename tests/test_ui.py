...
class TestLoginPage:
    ...
    def test_register_new_user(self, browser):
        ...
        try:
            ...
            assert (
                "success" in message.text.lower()
                or "registered" in message.text.lower()
            )
        except TimeoutException:
    ...
    def test_login_existing_user(self, browser):
        ...
        try:
            WebDriverWait(browser, 5).until(
                ...
                or EC.text_to_be_present_in_element((By.ID, "message"), "registered")
            )
        except TimeoutException:
            pass
...
class TestGameInterface:
    ...
    def test_game_interface_loads(self, logged_in_browser):
        ...
        for element_id in main_elements:
            ...
            ), f"Element with ID {element_id} is not displayed"
            ...
            if len(options) <= 1:  # Only the default "Select Character" option
                ...
                try:
                    ...
                    browser.find_element(
                        By.CSS_SELECTOR, "select#race option[value='HUMAN']"
                    ).click()
                    browser.find_element(
                        By.CSS_SELECTOR, "select#class option[value='FIGHTER']"
                    ).click()
                    ...
                    WebDriverWait(browser, 5).until(
                        lambda driver: len(
                            driver.find_elements(
                                By.CSS_SELECTOR, "#charactersDropdown option"
                            )
                        )
                        > 1
                    )
                    ...
                    pytest.skip(
                        " +
            "Character creation interface not implemented or different"from expected"
                    )
    ...
    def test_command_input(self, logged_in_browser):
        ...
        try:
            ...
            assert any(
                cmd in game_output.text.lower()
                for cmd in ["commands", "available", "options"]
            )
        except TimeoutException:
        ...
        try:
            ...
            WebDriverWait(browser, 5).until(
                EC.presence_of_element_located(
                    (By.CSS_SELECTOR, "#inventoryList .inventory-item")
                )
            )

            inventory_items = browser.find_elements(
                By.CSS_SELECTOR, "#inventoryList .inventory-item"
            )
            ...
            try:
                ...
                WebDriverWait(browser, 5).until(
                    EC.text_to_be_present_in_element(
                        (By.ID, "equippedItems"), "Equipped"
                    )
                    or EC.presence_of_element_located(
                        (By.CSS_SELECTOR, ".equipped-item")
                    )
                )

                # Check equipped items
                equipped_items = browser.find_elements(
                    By.CSS_SELECTOR, ".equipped-item"
                )
                ...
                try:
                    ...
                    WebDriverWait(browser, 3).until(
                        EC.presence_of_element_located(
                            (By.CSS_SELECTOR, ".inventory-item.equipped")
                        )
                    )
                    equipped = browser.find_elements(
                        By.CSS_SELECTOR, ".inventory-item.equipped"
                    )
                    assert len(equipped) > 0
                except Exception:
                    pytest.skip(
                        "Couldn't verify item was equipped - UI implementation may differ"
                    )
