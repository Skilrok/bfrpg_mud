    ...
    try:
        ...
        for character in characters:
            ...
            if current_class in [cls.name for cls in CharacterClass]:
                logger.info(
                    f"Character '{char_name}' (ID: {char_id}) already has correct class format: '{current_class}'"
                )
                continue
            ...
            if current_class in class_map:
                ...
                logger.warning(
                    f"Character '{char_name}' (ID: {char_id}) has class value '{current_class}' "
        ...
        for character in characters:
            ...
            if current_class not in [cls.name for cls in CharacterClass]:
                invalid_count += 1
                logger.error(
                    f"Character '{char_name}' (ID: {char_id}) still has invalid class: '{current_class}'"
                )
        ...
        if invalid_count == 0:
            ...
            logger.error(
                f"Found {invalid_count} characters with still invalid class values after fix attempt"
            )
