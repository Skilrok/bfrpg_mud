    ...
    with get_db_context() as db:
        try:
            ...
            for char_row in chars:
                char_id, char_name, race, char_class, inventory_json, equipment_json = char_row
                logger.info(f"Character: {char_name} (ID: {char_id}, Race: {race}, Class: {char_class})")
                ...
                logger.info(f"  Character items: {char_items_count} total, {equipped_count} equipped")
                
                # Show some items
                for i, (item_id, quantity, is_equipped, equip_slot) in enumerate(char_items[:3]):
                    # Get item name
                    item_name = db.execute(text("""
                        SELECT name FROM items WHERE id = :item_id
                    """), {"item_id": item_id}).scalar() or f"Item ID: {item_id}"
                    
                    equipped_str = f"equipped in {equip_slot}" if is_equipped else "not equipped"
                    logger.info(f"  Item {i+1}: {item_name} (quantity: {quantity}, {equipped_str})")
                
                # Verify inventory vs character_items consistency
                if inventory_count != char_items_count:
                    logger.warning(f"  ⚠️ Inconsistency: {inventory_count} items in inventory JSON vs {char_items_count} in character_items table")
                
                if equipment_count != equipped_count:
                    logger.warning(f"  ⚠️ Inconsistency: {equipment_count} items in equipment JSON vs {equipped_count} equipped in character_items table")
...
def run_migrate_verify():
    ...
    with get_db_context() as db:
        try:
            # Get all characters
            characters_raw = db.execute(text("""
            ...
            for char_row in characters_raw:
                ...
                char_items_raw = db.execute(text("""
                    SELECT item_id, quantity, is_equipped, equip_slot
                    FROM character_items 
                    WHERE character_id = :char_id
                """), {"char_id": char_id}).fetchall()
                    ...
                    for slot, item_id in old_equipment.items():
                        if slot not in new_equipment or new_equipment[slot] != item_id:
                            ...
                            if slot in new_equipment:
                                logger.warning(f"  Actual: {slot} -> (ID: {new_equipment[slot]})")
                            else:
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Check characters directly from database")
    parser.add_argument("--verify", action="store_true", help="Run verification of migration")
