...
with get_db_context() as db:
    ...
    if chain_mail:
        ...
        if chain_mail.properties:
            if "armor_class" in chain_mail.properties:
                print(f"  - armor_class from properties: {chain_mail.properties['armor_class']}")
            elif "base_ac" in chain_mail.properties:
                print(f"  - base_ac from properties: {chain_mail.properties['base_ac']}")
    else:
    ...
    for armor in armor_items:
        print(f"  - {armor.name}: AC={armor.armor_class}, properties={armor.properties}")
        
    print("\nAll shield items:")
    shield_items = db.query(models.Item).filter(models.Item.item_type == "shield").all()
    for shield in shield_items:
        print(f"  - {shield.name}: AC={shield.armor_class}, properties={shield.properties}") 