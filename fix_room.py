    ...
    try:
        ...
        if not room_exists:
            ...
            try:
                ...
                logger.info(
                    "Created additional rooms and exits around the village square"
                )
...
def fix_look_command():
    ...
    try:
        ...
        if look_cmd:
            logger.info(
                f"Look command found: {look_cmd.name} with aliases {look_cmd.aliases}"
            )
        else:
...
def create_missing_endpoints():
    ...
    try:
        ...
        logger.info(
            "Missing endpoints should be implemented in app/routers/characters.py"
        )
        return True
    except Exception as e:
...
if __name__ == "__main__":
    ...
    logger.info(
        f"Room creation result: {'Success' if room_result else 'Failed or already exists'}"
    )
    ...
    logger.info(
        f"Endpoints check result: {'Success' if endpoints_result else 'Failed'}"
    )
