    ...
    try:
        ...
        for character in characters:
            ...
            if current_race in [race.name for race in CharacterRace]:
                logger.info(f"Character '{char_name}' (ID: {char_id}) already has correct race format: '{current_race}'")
                continue
            ...
            if current_race in race_map:
                ...
                logger.warning(f"Character '{char_name}' (ID: {char_id}) has race value '{current_race}' "
        ...
        for character in characters:
            ...
            if current_race not in [race.name for race in CharacterRace]:
                invalid_count += 1
                logger.error(f"Character '{char_name}' (ID: {char_id}) still has invalid race: '{current_race}'")
                
        if invalid_count == 0:
            logger.info("All character races are now valid enum values")
        else:
            logger.error(f"Found {invalid_count} characters with still invalid race values after fix attempt")
