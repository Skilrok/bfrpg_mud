...
from sqlalchemy import text, Table, Column, Integer, String, Boolean, ForeignKey, MetaData, JSON, DateTime
...
from app.models.room import Area, Room, RoomType, CharacterLocation
...
async def create_test_character(session: AsyncSession, user_id: int) -> int:
    ...
    try:
        ...
        result = await session.execute(
            text("SELECT id FROM characters WHERE user_id = :user_id AND name = 'TestChar' LIMIT 1"),
            {"user_id": user_id}
        )
...
def test_character_location(character_id):
    ...
    try:
        ...
        if location_data:
            logger.info(f"Character {character_id} already has location {location_data[0]} in room {location_data[1]}")
...
def test_character_location_in_look_command(character_id):
    ...
    try:
        ...
        if location_data:
            logger.info(f"Character {character_id} is in room {location_data[0]} (direct SQL)")
        else:
        ...
        char_location = db.query(CharacterLocation).filter(CharacterLocation.character_id == character_id).first()
        
        if char_location:
            logger.info(f"Character {character_id} is in room {char_location.room_id} (ORM)")
            
            # Try to get room through relationship
            if char_location.room:
                logger.info(f"Room through relationship: ID={char_location.room.id}, Name={char_location.room.name}")
            else:
...
async def test_character_location():
    ...
    try:
        async with engine.begin() as conn:
            ...
            if location:
                logger.info(f"Character ID: {location['character_id']} is in "
                          f"Room: {location['room_name']} (ID: {location['room_id']}) in "
                          f"Area: {location['area_name']} (ID: {location['area_id']})")
            else:
                logger.error(f"Character location not found for character ID: {character_id}")
...
async def test_character_location_in_look_command(session, character_id):
    ...
    try:
        ...
        if room:
            ...
            if character:
                logger.info(f"Character info: {character['name']} - "
