...
def test_character_state_and_deletion():
    ...
    if not characters:
        ...
        create_response = requests.post(
            f"{API_URL}/api/characters", json=character_data, headers=auth_header
        )
        ...
        characters_response = requests.get(
            f"{API_URL}/api/characters", headers=auth_header
        )
        if characters_response.status_code != 200:
            logger.error(
                f"Failed to get characters after creation: {characters_response.text}"
            )
            return False
    ...
    logger.info(
        f"Using character: {character['name']} (ID: {character_id}) for state update test"
    )
    ...
    logger.info(
        f"Character state updated: Level={updated_character['level']}, HP={updated_character['hit_points']}, Gold={updated_character['gold']}"
    )
    ...
    create_response = requests.post(
        f"{API_URL}/api/characters", json=del_character_data, headers=auth_header
    )

    if create_response.status_code != 201:
        logger.error(
            f"Failed to create character for deletion test: {create_response.text}"
        )
        return False
    ...
    logger.info(
        f"Created character for deletion: {del_character['name']} (ID: {del_character_id})"
    )

    # Delete the character
    delete_response = requests.delete(
        f"{API_URL}/api/characters/{del_character_id}", headers=auth_header
    )
    ...
    if get_response.status_code == 404:
        ...
        logger.error(
            f"Character deletion verification failed: {get_response.status_code}"
        )
        return False
