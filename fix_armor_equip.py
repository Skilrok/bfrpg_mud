...
def fix_and_test_armor(character_name):
    ...
    with get_db_context() as db:
        try:
            ...
            character = db.query(models.Character).filter(models.Character.name == character_name).first()
            ...
            for item_id_str, item_data in inventory.items():
                try:
                    if item_data.get("equipped", False):  # Only check equipped items
                        ...
                        item = db.query(models.Item).filter(models.Item.id == item_id).first()
                        if item and item.item_type and item.item_type.value == "armor":
                            print(f"Found armor item {item_id} ({item.name}) marked as equipped in inventory")
                            ...
                            for slot_name, equip_item_id in equipment.items():
                                if equip_item_id == item_id:
                                    ...
                                    armor_item_id = item_id
                                    print(f"Armor is properly in equipment slot: {slot_name}")
                                    break
                            
                            # If not in equipment, add it
                            if not is_in_equipment:
                                print(f"Fixing: Adding armor to equipment in 'body' slot")
            ...
            if fixed:
                ...
                armor_item = db.query(models.Item).filter(models.Item.id == armor_item_id).first()
                ...
                for slot, item_id in equipment.items():
                    if slot == "off_hand":
                        shield_item = db.query(models.Item).filter(models.Item.id == item_id).first()
            ...
            if not armor_slot:
                ...
                if chain_mail_id_str in inventory and inventory[chain_mail_id_str].get("equipped", False):
                    print("Chain Mail found in inventory marked as equipped but not in equipment")
                    armor_item = db.query(models.Item).filter(models.Item.id == chain_mail_id).first()
                    armor_slot = "body"
            ...
            for slot, item_id in character.equipment.items():
                ...
                item = db.query(models.Item).filter(models.Item.id == item_id).first()
                if item and item.item_type and item.item_type.value == "shield" and item.properties:
            ...
            if character.armor_class == expected_ac:
                print("SUCCESS: Armor class was correctly updated when unequipping armor!")
                unequip_successful = True
            else:
                print(f"ERROR: AC update failed! Expected {expected_ac}, got {character.armor_class}")
            ...
            if character.armor_class == expected_ac:
                print("SUCCESS: Armor class was correctly updated when equipping armor!")
