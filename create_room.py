...
def create_starter_area():
    ...
    try:
        ...
        try:
            ...
            areas = db.execute(text("SELECT id, name FROM areas WHERE name = 'Starting Village' LIMIT 1")).fetchall()
            if areas:
                starting_area_id = areas[0][0]
                starting_area_name = areas[0][1]
                logger.info(f"Starting area already exists: {starting_area_name} (ID: {starting_area_id})")
            else:
            ...
            rooms = db.execute(text("SELECT id, name FROM rooms WHERE id = 1 LIMIT 1")).fetchall()
            if rooms:
                starting_room_id = rooms[0][0]
                starting_room_name = rooms[0][1]
                logger.info(f"Starting room already exists: {starting_room_name} (ID: {starting_room_id})")
            else:
