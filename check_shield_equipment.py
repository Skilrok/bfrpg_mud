...
def check_shield_equipment():
    ...
    try:
        ...
        character_items = (
            ...
            .filter(CharacterItem.item_id == 80)
            .all()
        )
        
        if not character_items:
            logger.info("No characters have shield (item ID 80) in their inventory or equipment")
            return
        ...
        for char_item in character_items:
            character = db.query(Character).filter(Character.id == char_item.character_id).first()
            if not character:
                logger.warning(f"Character {char_item.character_id} not found but has shield item")
                continue
            ...
            if character.equipment:
                for slot, item_id in character.equipment.items():
                    if item_id == 80:
                        logger.warning(f"  Shield found in equipment JSON field in slot: {slot}")
