...
def create_test_character(token):
    ...
    response = requests.post(
        f"{BASE_URL}/api/characters/", json=character_data, headers=headers
    )

    if response.status_code != 200:
        print(
            f"Failed to create test character: {response.status_code} - {response.text}"
        )
        sys.exit(1)
...
def get_character_inventory(token, character_id):
    ...
    for item_id, item_data in inventory.items():
        print(
            f"- {item_data.get('name', 'Unknown')} (x{item_data['quantity']}) [ID: {item_id}]"
        )
...
def get_character_details(token, character_id):
    ...
    response = requests.get(
        f"{BASE_URL}/api/characters/{character_id}", headers=headers
    )

    if response.status_code != 200:
        print(
            f"Failed to get character details: {response.status_code} - {response.text}"
        )
        return None
...
def add_item_to_inventory(token, character_id, item_id, quantity=1):
    ...
    request_data = {"item_id": item_id, "quantity": quantity}
    ...
    if response.status_code != 200:
        print(
            f"Failed to add item to inventory: {response.status_code} - {response.text}"
        )
        return False
...
def equip_item(token, character_id, item_id, slot):
    ...
    request_data = {"item_id": item_id, "slot": slot}
