            ...
            for item in items:
                ...
                if item.properties and "ac_bonus" in item.properties:
                    try:
                        ...
                        logger.info(
                            f"Set ac_bonus={ac_bonus_value} for item id={item.id}, name='{item.name}'"
                        )
                        count += 1
                    except (ValueError, TypeError) as e:
                        logger.error(
                            f"Error converting ac_bonus for item {item.id}: {e}"
                        )
...
def verify_migration():
    ...
    with Session(engine) as session:
        ...
        logger.info(
            f"  - properties value: {shield.properties.get('ac_bonus') if shield.properties else None}"
        )
