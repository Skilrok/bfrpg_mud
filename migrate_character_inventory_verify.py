    ...
    with get_db_context() as db:
        try:
            ...
            for character in characters:
                ...
                character_items = db.query(CharacterItem).filter(
                    CharacterItem.character_id == character.id
                ).all()
                
                logger.info(f"Character: {character.name} (ID: {character.id}) - {len(character_items)} items")
                ...
                for i, item in enumerate(character_items[:3]):  # Show first 3 items
                    item_name = item.item.name if hasattr(item, 'item') and item.item else f"Item ID: {item.item_id}"
                    equipped_str = f"equipped in {item.equip_slot}" if item.is_equipped else "not equipped"
                ...
                if hasattr(character, 'inventory'):
                    ...
                    if len(inventory) == len(character_items):
                        logger.info("  ✅ inventory property length matches CharacterItems count")
                    else:
                        logger.warning(f"  ❌ inventory property length mismatch: {len(inventory)} vs {len(character_items)}")
                else:
                ...
                if hasattr(character, 'equipment'):
                    ...
                    logger.info(f"  equipment property: {len(equipment)} equipped items")
                    
                    # Verify equipment matches equipped CharacterItems
                    if len(equipment) == len(equipped_items):
                        logger.info("  ✅ equipment property length matches equipped CharacterItems count")
                    else:
                        logger.warning(f"  ❌ equipment property length mismatch: {len(equipment)} vs {len(equipped_items)}")
                else:
