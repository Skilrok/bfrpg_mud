...
def verify_equip_command():
    ...
    try:
        ...
        if re.search(updated_pattern, content):
            ...
            logger.warning("❌ EquipCommand has NOT been updated to use ac_bonus column")
            return False
...
def verify_character_router():
    ...
    try:
        ...
        if re.search(updated_pattern, content):
            ...
            logger.warning("❌ Character router has NOT been updated to use ac_bonus column")
            return False
    ...
    try:
        ...
        with get_db_context() as db:
            ...
            if shield.ac_bonus is not None:
                logger.info(f"✅ Shield item (ID 80) has ac_bonus column populated with value: {shield.ac_bonus}")
                return True
            else:
                logger.warning("❌ Shield item (ID 80) does NOT have ac_bonus column populated")
                return False
