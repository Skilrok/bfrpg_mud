...
def check_and_fix_rooms():
    ...
    try:
        ...
        if not room_1:
            ...
            "A cozy tavern with a roaring fireplace. Adventurers gather"here to share tales.",
                ...
                (
                    3,
                    "Village Market",
                    "A bustling market with various stalls selling goods.",
                    "town",
                    area_id,
                    1,
                    0,
            ...
            "The sound of hammering fills the air as the blacksmith"works at the forge.",
                ...
                (
                    5,
                    "Village Temple",
                    "A peaceful temple dedicated to various deities.",
                    "town",
                    area_id,
                    -1,
                    0,
            ...
            exits_data = [
                ...
                (
                    ...
                    "north",
                    "Tavern Entrance",
                    "The door to the village tavern.",
                    2,
                    0,
                    0,
                ),
        ...
        # Get all exits
        cursor.execute(
            "SELECT source_room_id, direction, destination_room_id FROM exits"
        )
        ...
        cursor.execute(
            "SELECT character_id FROM character_locations WHERE character_id = 1"
        )
...
"""
        ...
        if start_marker in content:
            ...
            next_method_pos = content.find(
                next_method_marker, execute_start + len(start_marker)
            )
            register_pos = content.find(
                register_marker, execute_start + len(start_marker)
            )

            # Determine where the execute method ends
            if next_method_pos != -1 and (
                register_pos == -1 or next_method_pos < register_pos
            ):
...
if __name__ == "__main__":
    ...
    logger.info(
        f"Look command modification {'successful' if look_result else 'failed'}"
    )
