    ...
    try:
        ...
        try:
            locations = db.execute(
                text("SELECT character_id, room_id FROM character_locations")
            ).fetchall()
            if locations:
        ...
        try:
            ...
            if exits:
                ...
                for exit in exits:
                    logger.info(
                        f"  Exit from room {exit[0]} {exit[1]} to room {exit[2]}"
                    )
            else:
...
def fix_character_location(character_id=1):
    ...
    try:
        ...
        if location:
            logger.info(
                f"Character {character_id} already has location: room {location[1]}"
            )
            # Update to room 1
            db.execute(
                text(
...
# REMOVED:         from app.commands.registry import command_registry
        ...
        if look_command:
            logger.info(
                f"Found look command: {look_command.name} with aliases {look_command.aliases}"
            )
            ...
            if hasattr(look_command, "_look_at_room"):
                ...
                async def debug_look_at_room(self, db, character_id):
                    ...
                    try:
                        ...
                        logger.info(
                            f"DEBUG: Look result message: {result.message[:100]}..."
                        )
                        return result
                    except Exception as e:
                ...
                look_command._look_at_room = MethodType(
                    debug_look_at_room, look_command
                )
