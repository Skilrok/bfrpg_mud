...
def check_database():
    ...
    with get_db_context() as db:
        try:
            ...
            try:
                item_count = db.execute(text("SELECT COUNT(*) FROM character_items")).scalar()
                ...
                char = db.execute(text("SELECT id, name FROM characters LIMIT 1")).first()
                if char:
                    ...
                    items = db.execute(
                        text("SELECT item_id, is_equipped, equip_slot FROM character_items WHERE character_id = :char_id"),
                        {"char_id": char_id}
                    ).fetchall()
                    ...
                    for i, (item_id, is_equipped, equip_slot) in enumerate(items[:5]):
                        ...
                        logger.info(f"  Item {i+1}: ID {item_id} - {item_name} (equipped: {is_equipped}, slot: {equip_slot})")
                else:
                    logger.warning("No characters found in database")
                
            except Exception as e:
                logger.error(f"Error checking character_items table: {str(e)}")
                logger.info("The character_items table may not exist yet - run the migration first")
    ...
    try:
        ...
        for char in characters:
            logger.info(f"ID: {char.id}, Name: {char.name}, Race: {char.race}, Class: {char.character_class}")
