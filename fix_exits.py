...
def create_exits():
    ...
    try:
        ...
        for exit_data in exits_data:
            ...
            """, (
                source_id, direction, name, description, 
                dest_id, is_hidden, is_locked, properties,
                now, now
            ))
...
"""
        ...
        modified_content = content[:class_end_pos] + debug_function + content[class_end_pos:]
    ...
    try:
        ...
        try:"""
            ...
            await self.send_error(websocket, f"Error: {str(e)}")"""
                ...
                logger.warning("Could not find handle_command_message in WebSocket manager")
...
if __name__ == "__main__":
    ...
    logger.info(f"Command handling debug {'successful' if handler_result else 'failed'}")
