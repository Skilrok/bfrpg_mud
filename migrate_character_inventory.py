...
from app.models import Character, CharacterClass, CharacterItem, CharacterRace, Item
    ...
    try:
        ...
        for row in result:
            ...
            if fixed_race or fixed_class:
                ...
                if fixed_race:
                    logger.info(
                        f"Fixing character {char_id} race: '{race}' → '{fixed_race}'"
                    )
                    update_fields["race"] = fixed_race

                if fixed_class:
                    logger.info(
                        f"Fixing character {char_id} class: '{char_class}' → '{fixed_class}'"
                    )
...
def migrate_character_inventory():
    ...
    with get_db_context() as db:
        try:
            ...
            if existing_items > 0:
                logger.warning(
                    f"Found {existing_items} existing character items - clearing before migration"
                )
            ...
            try:
                ...
                logger.error(
                    f"CharacterItem table does not exist or cannot be queried: {str(e)}"
                )
                logger.error(
                    "Please run the Alembic migration first: alembic upgrade a35b9c72e451"
                )
                return False
            ...
            for char_row in characters_raw:
                ...
                try:
                    ...
                    try:
                        ...
                        logger.warning(
                            f"Invalid JSON data for character {char_name} (ID: {char_id}) - skipping"
                        )
                    ...
                    logger.info(
                        f"Migrating inventory for character: {char_name} (ID: {char_id})"
                    )
                    ...
                    if isinstance(inventory, dict):
                        for item_id_str, item_data in inventory.items():
                            try:
                                ...
                                if not item:
                                    logger.warning(
                                        f"Item ID {item_id} not found in database - skipping"
                                    )
                                    continue
                                ...
                                logger.info(
                                    f"Added item {item_id} to character {char_name} (equipped: {is_equipped}, slot: {equip_slot})"
                                )
                    ...
                    if isinstance(equipment, dict):
                        for slot, item_id in equipment.items():
                            try:
                                logger.info(
                                    f"Processing equipment item in slot {slot}: {item_id}"
                                )

                                # Skip if we already processed this item
                                if item_id in processed_items:
                                    logger.info(
                                        f"Item {item_id} already processed from inventory"
                                    )
                                    continue
                                ...
                                if not item:
                                    logger.warning(
                                        f"Equipment item ID {item_id} not found in database - skipping"
                                    )
                                    continue
                                ...
                                total_items += 1
                                logger.info(
                                    f"Added equipment item {item_id} in slot {slot} to character {char_name}"
                                )

                            except Exception as e:
                                logger.error(
                                    f"Error processing equipment item {item_id} in slot {slot}: {str(e)}"
                                )
                                traceback.print_exc()
                    ...
                    verify_character_migration(
                        db, char_id, char_name, inventory, equipment
                    )

                except Exception as e:
                    logger.error(
                        f"Error processing character {char_name} (ID: {char_id}): {str(e)}"
                    )
                    traceback.print_exc()
                    db.rollback()

            logger.info(
                f"Migration complete. Migrated {migrated_characters}/{total_characters} characters with {total_items} total items."
            )
            return True
...
def verify_character_migration(db, char_id, char_name, old_inventory, old_equipment):
    ...
    char_items = (
        db.query(CharacterItem).filter(CharacterItem.character_id == char_id).all()
    )
        ...
        for slot, item_id in old_equipment.items():
            if slot not in new_equipment:
                ...
                equipment_issues.append(
                    f"Item mismatch in slot {slot}: expected {item_id}, got {new_equipment[slot]}"
                )
        ...
        for slot, item_id in old_equipment.items():
            if slot not in new_equipment or new_equipment[slot] != item_id:
                try:
                    ...
                    if not item:
                        logger.warning(
                            f"Cannot fix: Item {item_id} not found in database"
                        )
                        continue

                    logger.info(
                        f"Fixing equipment: {item.name} (ID: {item_id}) in slot {slot}"
                    )
                    ...
                    if existing_item:
                        ...
                        # Create new item
                        logger.info(
                            f"Creating new equipped item {item_id} in slot {slot}"
                        )
                        char_item = CharacterItem(
                            character_id=char_id,
                            item_id=item_id,
    ...
    if isinstance(old_inventory, dict) and len(old_inventory) > len(new_inventory):
        logger.warning(
            f"Inventory size mismatch - old: {len(old_inventory)}, new: {len(new_inventory)}"
        )
    ...
    with get_db_context() as db:
        try:
            ...
            for char_row in characters_raw:
                ...
                char_items = (
                    db.query(CharacterItem)
                    .filter(CharacterItem.character_id == char_id)
                    .all()
                )
                ...
                if isinstance(old_inventory, dict):
                    for item_id_str, item_data in old_inventory.items():
                        if item_id_str not in new_inventory:
                            ...
                            logger.warning(
                                f"Item {item_id_str} missing from new inventory for character {char_name}"
                            )
                        else:
                            # Check item details
                            ...
                            if (
                                ...
                                or new_item["equipped"]
                                != item_data.get("equipped", False)
                                or new_item["slot"] != item_data.get("slot")
                            ):
                                inventory_match = False
                                logger.warning(
                                    f"Item {item_id_str} details mismatch for character {char_name}"
                                )
                    ...
                    for slot, item_id in old_equipment.items():
                        if slot not in new_equipment or new_equipment[slot] != item_id:
                            ...
                            if slot in new_equipment:
                                new_item = (
                                    db.query(Item)
                                    .filter(Item.id == new_equipment[slot])
                                    .first()
                                )
                ...
                if inventory_match and equipment_match:
                    ...
                    if not equipment_match:
                        ...
                        verify_character_migration(
                            db, char_id, char_name, old_inventory, old_equipment
                        )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Migrate character inventory data to the new table structure"
    )
    parser.add_argument(
        "--verify",
        action="store_true",
        help="Verify migration results without performing migration",
    )
