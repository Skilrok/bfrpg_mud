...
from app.models import CharacterRace, CharacterClass, Character
from app.schemas.character import CharacterCreate
from app.database import SessionLocal, engine
from app.commands.character_commands import calculate_racial_abilities, calculate_saving_throws
...
def test_character_creation():
    ...
    try:
        ...
        if character_model.race in [CharacterRace.HALFLING, CharacterRace.ELF] and hit_die > 6:
            hit_die = 6
        ...
        saves = calculate_saving_throws(character_model.character_class, 1, character_model.race)
        ...
        thief_abilities = {}
        if character_model.character_class in [CharacterClass.THIEF, CharacterClass.MAGIC_USER_THIEF]:
            thief_abilities = {
                "open_locks": 25,
                "remove_traps": 20,
                "pick_pockets": 30,
        ...
        logger.info(f"Creating character: {character_model.name} (Race: {character_model.race}, Class: {character_model.character_class})")
...
if __name__ == "__main__":
    ...
    if success:
        logger.info(f"Test successful! Character created: {result.name} (ID: {result.id})")
    else:
