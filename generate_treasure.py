...
LAIR_TREASURE_TABLES = {
    ...
    "V": {
        "magic_items": {"chance": 100, "items": "Any 2"}    # Any 2 magic items
    }
}
...
GEM_VALUES = [
    {"value": 10, "types": ["Quartz", "Citrine", "Rock Crystal", "Turquoise", "Hematite"]},
    {"value": 50, "types": ["Amber", "Amethyst", "Garnet", "Jade", "Jasper", "Pearl", "Spinel"]},
    {"value": 100, "types": ["Aquamarine", "Alexandrite", "Topaz", "Opal", "Coral", "Tourmaline"]},
    {"value": 500, "types": ["Ruby", "Emerald", "Sapphire", "Diamond", "Star Ruby", "Star Sapphire"]}
]
...
def generate_jewelry() -> Dict:
    ...
    jewelry_types = [
        "Ring", "Necklace", "Bracelet", "Brooch", "Crown", 
        "Tiara", "Earrings", "Anklet", "Medallion", "Amulet"
    ]
    
    materials = [
        "Gold", "Silver", "Platinum", "Electrum", 
        "Bronze", "Copper", "Ivory", "Pearl-studded"
    ]
...
def create_magic_weapon() -> Dict:
    ...
    weapons = [
        ...
        "Bow", "Arrow", "Warhammer", "Flail", "Staff"
    ]
...
def create_magic_potion() -> Dict:
    ...
    potion_types = [
        "Healing", "Extra Healing", "Fire Resistance", "Cold Resistance",
        "Invisibility", "Gaseous Form", "Speed", "Strength", "Growth", 
        "Diminution", "Flying", "Animal Control", "Undead Control", 
        "Plant Control", "Giant Control"
    ]
...
def create_magic_scroll() -> Dict:
    ...
    if roll <= 70:
        ...
        spells = random.sample(spell_levels[level], min(num_spells, len(spell_levels[level])))
...
def create_magic_ring() -> Dict:
    ...
    ring_types = [
        "Protection +1", "Protection +2", "Fire Resistance", 
        "Invisibility", "Regeneration", "Water Walking", 
        "Wishes", "X-Ray Vision", "Animal Control"
    ]
...
def create_magic_wand() -> Dict:
    ...
    wand_types = [
        "Magic Detection", "Enemy Detection", "Illusion", 
        "Fear", "Fireballs", "Lightning Bolts", "Cold", 
        "Paralyzation", "Polymorph"
    ]
...
def create_miscellaneous_magic() -> Dict:
    ...
    misc_items = [
        "Bag of Holding", "Boots of Speed", "Boots of Levitation", 
        "Crystal Ball", "Cloak of Displacement", "Gauntlets of Ogre Power", 
        "Helm of Telepathy", "Rope of Climbing", "Medallion of ESP", 
        "Broom of Flying"
    ]
    ...
    for part in parts:
        ...
        if dice_match:
            ...
            if item_type == "any":
                for _ in range(count):
                    items.append(determine_magic_item_type(category))
            elif item_type in ["weapon", "armor", "potion", "scroll", "ring", "wand", "misc"]:
            ...
            if simple_match:
                ...
                if item_type in ["weapon", "armor", "potion", "scroll", "ring", "wand", "misc"]:
                    items.extend([item_type] * count)
                elif item_type == "any":
                    for _ in range(count):
...
def add_treasure_to_database(treasure: Dict, owner_id: int, session) -> None:
    ...
    if treasure["coins"]["cp"] > 0 or treasure["coins"]["sp"] > 0 or \
       treasure["coins"]["gp"] > 0 or treasure["coins"]["pp"] > 0:
        
        # Check if coins already exist in inventory
        coin_item = session.query(Item).filter(
            Item.name == "Coins",
            Item.item_type == ItemType.CURRENCY
        ).first()
        ...
        if inventory_item:
            ...
            coin_item.properties["cp"] = coin_item.properties.get("cp", 0) + treasure["coins"]["cp"]
            coin_item.properties["sp"] = coin_item.properties.get("sp", 0) + treasure["coins"]["sp"]
            coin_item.properties["gp"] = coin_item.properties.get("gp", 0) + treasure["coins"]["gp"]
            coin_item.properties["pp"] = coin_item.properties.get("pp", 0) + treasure["coins"]["pp"]
        else:
    ...
    for magic_item in treasure["magic_items"]:
        ...
        item = Item(
            ...
            equip_slot=magic_item["equip_slot"] if magic_item["is_equippable"] else None,
...
def main():
    ...
    # Check if code exists in either table
    if treasure_code not in INDIVIDUAL_TREASURE_TABLES and treasure_code not in LAIR_TREASURE_TABLES:
