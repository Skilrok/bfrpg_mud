        ...
        if os.path.exists(init_file):
            ...
            if "import app.commands.debug_command" not in init_content:
                ...
                if init_content.strip():
                    ...
                    init_content = "# Import debug command\nimport app.commands.debug_command\n"
...
if __name__ == "__main__":
    logger.info("Starting WebSocket and command fixes...")
    
    # Step 1: Fix WebSocket handler
    logger.info("Step 1: Adding debug info to WebSocket handler...")
    ws_result = fix_websocket_handler()
    logger.info(f"WebSocket handler fix result: {'Success' if ws_result else 'Failed'}")
    
    # Step 2: Fix look command
    logger.info("Step 2: Adding debug info to look command...")
    ...
    logger.info("Step 3: Creating debug command...")
    cmd_result = create_debug_command()
    logger.info(f"Debug command creation result: {'Success' if cmd_result else 'Failed'}")
    
    # Step 4: Fix JavaScript WebSocket handling
    logger.info("Step 4: Adding debug info to JavaScript WebSocket handling...")
    js_result = fix_js_websocket()
    logger.info(f"JavaScript WebSocket fix result: {'Success' if js_result else 'Failed'}")
    
    logger.info("Fixes completed, please restart the server to apply changes") 