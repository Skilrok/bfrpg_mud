...
class CommandRegistry:
    ...
    def list_commands(
        ...
        if category:
            return [
                cmd for cmd in self.commands.values() if cmd["category"] == category
            ]
...
class CommandExecutor:
    ...
    def check_requirements(
        requirements: List[CommandRequirement], context: Dict[str, Any]
    ) -> tuple[bool, str]:
        """
        ...
        if CommandRequirement.HAS_CHARACTER in requirements and not context.get(
            "character_id"
        ):
        ...
        if CommandRequirement.ADMIN in requirements and not context.get(
            "is_admin", False
        ):
    ...
    @staticmethod
    async def execute(
        cmd_name: str, args: List[str], context: Dict[str, Any]
    ) -> CommandResult:
        """
        ...
        if not requirements_met:
            return CommandResult(
                success=False, message=error_message, error="REQUIREMENTS_NOT_MET"
            )
