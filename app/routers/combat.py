...
async def initiate_combat(
    ...
    combat_session = CombatSession(character_id=character_id, monster_data=monster_data)
...
async def roll_initiative(
    ...
    if combat_id not in active_combats:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail="Combat session not found"
        )
    ...
    if character_initiative > monster_initiative:
        first = "character"
        combat.log.append(
            f"{character.name} wins initiative with {character_initiative} vs. {monster_initiative}"
        )
    elif monster_initiative > character_initiative:
        first = "monster"
        combat.log.append(
            f"Monster ({combat.monster_data['name']}) wins initiative with {monster_initiative} vs. {character_initiative}"
        )
    else:
        # Tie - roll again
        first = "tie"
    ...
    if attacker_type == "character":
        ...
        if hit:
            ...
            if critical:
                damage *= 2
                combat.log.append(
                    f"{character.name} scores a critical hit for {damage} damage!"
                )
            else:
        ...
        if (
            character.race == models.CharacterRace.HALFLING
            and monster.get("size", "medium") != "small"
        ):
            target_ac += 2
        ...
        if hit:
            ...
            if critical:
                damage *= 2
                combat.log.append(
                    f"{monster['name']} scores a critical hit for {damage} damage!"
                )
            else:
