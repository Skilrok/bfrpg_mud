...
async def delete_area(
    ...
    try:
        ...
        room_count = (
            db.query(models.Room).filter(models.Room.area_id == area_id).count()
        )
        if room_count > 0:
            raise HTTPException(
...
async def get_area_rooms(
    ...
    try:
        ...
        rooms = (
            ...
            .filter(models.Room.area_id == area_id)
            .offset(skip)
            .limit(limit)
            .all()
        )
        ...
        logger.exception(
            f"Database error retrieving rooms for area {area_id}: {str(e)}"
        )
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
