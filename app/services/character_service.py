    ...
    try:
        ...
        try:
            room_exists = db.execute(
                text("SELECT id FROM rooms WHERE id = 1")
            ).fetchone()
        ...
        if not room_exists:
            ...
            if not area_result:
                ...
                try:
                    ...
                    db.execute(
                        text(
                            "SELECT 1 FROM information_schema.tables WHERE table_name = 'areas'"
                        )
                    ).fetchone()
        ...
        try:
            location_exists = db.execute(
                text(
                    "SELECT id FROM character_locations WHERE character_id = :char_id"
                ),
            ...
            if not check_location or check_location[0] != 1:
                logger.error(
                    f"Failed to verify character {character_id} location after commit"
                )
                return False
