...
class Exit(Base):
    ...
    )  # Hidden exit, not visible in room description
    ...
    source_room = relationship(
        "Room", foreign_keys=[source_room_id], back_populates="outgoing_exits"
    )
    destination_room = relationship(
        "Room", foreign_keys=[destination_room_id], back_populates="incoming_exits"
    )
