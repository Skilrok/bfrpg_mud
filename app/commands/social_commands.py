...
from app.models import NPC, Character, CharacterLocation, Room, RoomNPC
...
class SayCommand(CommandHandler):
    ...
    async def execute(self, ctx: CommandContext) -> CommandResponse:
        ...
        npcs = (
            db.query(NPC)
            .join(RoomNPC)
            .filter(RoomNPC.room_id == location.room_id)
            .all()
        )
...
class EmoteCommand(CommandHandler):
    ...
    async def execute(self, ctx: CommandContext) -> CommandResponse:
        ...
        return CommandResponse(
            success=True, message=emote_message, data={"action": action}
        )
...
class TalkCommand(CommandHandler):
    ...
    async def execute(self, ctx: CommandContext) -> CommandResponse:
        ...
        npcs = (
            ...
            .join(RoomNPC)
            .filter(
                RoomNPC.room_id == location.room_id, NPC.name.ilike(f"%{target_npc}%")
            )
            .all()
        )
        ...
        if topic and npc.dialogs and isinstance(npc.dialogs, dict):
            ...
            if topic in npc.dialogs:
                ...
                for key, value in npc.dialogs.items():
                    ...
                    response = npc.dialogs.get(
                        "greeting", "The NPC doesn't seem interested in that topic."
                    )
        else:
            # Default to greeting if no topic specified
            response = (
                npc.dialogs.get("greeting", "The NPC nods at you.")
                if npc.dialogs
                else "The NPC nods at you."
            )
