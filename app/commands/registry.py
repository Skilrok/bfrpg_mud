...
class CommandRegistry:
    ...
    def register(self, handler_class: Type[CommandHandler]) -> None:
        ...
        if not handler.name:
            logger.error(
                f"Cannot register command with empty name: {handler_class.__name__}"
            )
            return
        ...
        for alias in handler.aliases:
            if alias in self._command_aliases:
                logger.warning(
                    f"Command alias '{alias}' already registered, overwriting"
                )
            self._command_aliases[alias] = handler.name

        logger.info(
            f"Registered command '{handler.name}' with aliases: {handler.aliases}"
        )
    ...
    async def execute_command(self, ctx: CommandContext) -> CommandResponse:
        ...
        if not ctx.command:
            return CommandResponse(
                success=False, message="No command specified", errors=["Empty command"]
            )
