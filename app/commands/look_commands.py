...
from app.models import CharacterLocation, RoomItem, Room, Character
    ...
    async def _look_at_room(
        ...
        try:
            ...
            room_data = db.execute(
                text("SELECT id, name, description, room_type, area_id, is_dark, exits FROM rooms WHERE id = :room_id"),
                {"room_id": room_id}
            ).fetchone()
            ...
            room_width = min(max(len(room_name) + 10, 60), 80)  # Width between 60-80 chars
            description = f"\n{'-' * room_width}\n"
            ...
            exits_by_direction = {exit_data[0]: (exit_data[1], exit_data[2]) for exit_data in exits_data}
            ...
            if exits_by_direction:
                ...
                for i, row in enumerate(compass):
                    for direction, symbol in direction_symbols.items():
                        if direction in exits_by_direction:
                            ...
                            compass[i] = compass[i].replace(f"{{{symbol}}}", direction_text)
                        else:
                ...
                other_dirs = [d for d in exits_by_direction if d not in direction_symbols.keys()]
                if other_dirs:
            ...
            return CommandResponse(
                ...
                data={
                    "room_id": room_id,
                    "room_name": room_name,
                    "items": [{"id": item[0], "name": item[2], "quantity": item[1]} for item in items_data],
                    "npcs": [{"id": npc[0], "name": npc[1]} for npc in npcs_data],
                    "characters": [{"id": char[0], "name": char[1]} for char in characters_data],
    ...
    async def _look_at_target(
        ...
        if character_data:
            ...
            return CommandResponse(
                ...
                data={"player_name": player_name, "player_description": player_description},
            )
