    ...
    try:
        with get_db_context() as db:
            ...
            if shield.properties and "ac_bonus" in shield.properties:
                logger.info(
                    f"properties['ac_bonus'] value: {shield.properties['ac_bonus']}"
                )
            else:
            ...
            logger.info(
                "2. New column value (shield.ac_bonus) is not used in most places"
            )
            logger.info(
                "3. To fully utilize the new column, code would need to be updated"
            )
            ...
            if shield.ac_bonus is not None:
                ...
                if shield.properties and "ac_bonus" in shield.properties:
                    if shield.ac_bonus == shield.properties["ac_bonus"]:
                        logger.info(
                            "✅ The ac_bonus column value matches the properties value"
                        )
                    else:
                        logger.error(
