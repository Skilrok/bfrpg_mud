...
def test_armor_equip_cycle(character_name):
    ...
    with get_db_context() as db:
        logger.info(f"Starting armor equip/unequip cycle test for character: {character_name}")
        ...
        logger.info(f"Initial character state - AC: {initial_ac}, Equipment: {initial_equipment}")
        ...
        if has_equipped_armor:
            ...
            logger.info(f"Found equipped armor: {armor_item.name} with AC bonus: {armor_ac_bonus}")
            ...
            logger.info(f"Unequipping armor. Expected AC will be: {expected_ac_unequipped}")
            ...
            logger.info(f"After unequipping - AC: {character.armor_class}, Expected: {expected_ac_unequipped}")
            if character.armor_class != expected_ac_unequipped:
                logger.error(f"AC mismatch after unequipping: Got {character.armor_class}, Expected {expected_ac_unequipped}")
            ...
            if armor_in_inventory:
                logger.info(f"Found armor in inventory at position {armor_inventory_index}, re-equipping")
                
                # Expected AC after re-equipping
                expected_ac_reequipped = expected_ac_unequipped + armor_ac_bonus
                logger.info(f"Re-equipping armor. Expected AC will be: {expected_ac_reequipped}")
                
                # Re-equip the armor
                equip_item(character=character, inventory_index=armor_inventory_index, db=db)
                db.commit()
                db.refresh(character)
                
                # Verify AC after re-equipping
                logger.info(f"After re-equipping - AC: {character.armor_class}, Expected: {expected_ac_reequipped}")
                if character.armor_class != expected_ac_reequipped:
                    logger.error(f"AC mismatch after re-equipping: Got {character.armor_class}, Expected {expected_ac_reequipped}")
            else:
                logger.error(f"Could not find the unequipped armor in inventory to re-equip")
        else:
            logger.info("No armor currently equipped. Checking inventory for armor to equip.")
            ...
            if armor_found:
                logger.info(f"Found armor in inventory: {armor_item.name} at position {armor_inventory_index}")
                ...
                logger.info(f"Equipping armor. Expected AC will be: {expected_ac_equipped}")
                ...
                logger.info(f"After equipping - AC: {character.armor_class}, Expected: {expected_ac_equipped}")
                if character.armor_class != expected_ac_equipped:
                    logger.error(f"AC mismatch after equipping: Got {character.armor_class}, Expected {expected_ac_equipped}")
