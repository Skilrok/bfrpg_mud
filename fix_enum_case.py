...
def inspect_enum_values():
    ...
    with get_db_context() as db:
        try:
            ...
            classes = db.execute(
                text("SELECT DISTINCT character_class FROM characters")
            ).fetchall()
            ...
            for race in race_mismatches:
                chars = db.execute(
                    text("SELECT id, name FROM characters WHERE race = :race"),
                    {"race": race},
                ).fetchall()
                for char_id, char_name in chars:
                    logger.warning(
                        f"Character {char_name} (ID: {char_id}) has invalid race: '{race}'"
                    )
            ...
            for cls in class_mismatches:
                ...
                for char_id, char_name in chars:
                    logger.warning(
                        f"Character {char_name} (ID: {char_id}) has invalid class: '{cls}'"
                    )
            ...
            for row in result:
                ...
                if race and isinstance(race, str):
                    for enum_value in CharacterRace:
                        if (
                            race.upper() == enum_value.value.upper()
                            and race != enum_value.value
                        ):
                ...
                if char_class and isinstance(char_class, str):
                    for enum_value in CharacterClass:
                        if (
                            char_class.upper() == enum_value.value.upper()
                            and char_class != enum_value.value
                        ):
                            fixed_class = enum_value.value
                            logger.info(
                                f"  Fixing class: '{char_class}' → '{fixed_class}'"
                            )
                            break
        ...
        try:
            ...
            for lowercase, correct in race_map.items():
                if lowercase != correct:
                    ...
                    if result.rowcount > 0:
                        logger.info(
                            f"Fixed {result.rowcount} characters with race '{lowercase}' → '{correct}'"
                        )
            ...
            for lowercase, correct in class_map.items():
                if lowercase != correct:
                    ...
                    if result.rowcount > 0:
                        logger.info(
                            f"Fixed {result.rowcount} characters with class '{lowercase}' → '{correct}'"
                        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--inspect", action="store_true", help="Only inspect enum values without fixing"
    )
    parser.add_argument(
        "--force",
        action="store_true",
        help="Force fix all enum values using direct SQL UPDATE",
    )
