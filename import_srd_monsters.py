                ...
                if hd_match:
                    ...
                    if "/" in hit_dice:  # Handle fractions like 1/2
                        ...
                        if hd_parts:
                            ...
                            bonus = int(hd_parts.group(2) or 0) if hd_parts.group(2) else 0
                            penalty = int(hd_parts.group(3) or 0) if hd_parts.group(3) else 0
...
def create_monster_dict(name: str, stats: Dict, description: str) -> Dict:
    ...
    if "dragon" in name.lower():
        ...
        monster_type = "giant"
    elif "undead" in description.lower() or any(x in name.lower() for x in ["ghost", "ghoul", "skeleton", "zombie", "vampire"]):
        monster_type = "undead"
    ...
    # Add attacks
    if "attacks" in stats:
        properties["attacks"] = [{"name": attack.strip(), "damage": stats.get("damage", "")} 
    ...
    monster_dict = {
        "name": name,
        "description": description[:500] if len(description) > 500 else description,  # Truncate if too long
...
def import_monsters(db: Session, monsters: List[Dict]) -> None:
    ...
    for monster_dict in monsters:
        ...
        existing = db.query(NPC).filter(NPC.name == monster_dict["name"], 
                                       NPC.npc_type == NPCType.MONSTER).first()
        if existing:
